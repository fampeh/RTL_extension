...
const INTRINSIC_LTR_DESCENDANT_SELECTOR = INTRINSIC_LTR_SELECTORS
  .map(selector => `${selector} *`)
  .join(", ");
const EDITABLE_OR_FORM_SELECTOR = [
  "textarea",
  "input",
  "select",
  "option",
...
const SKIPPED_TEXT_CONTAINER_SELECTOR = [
  EDITABLE_OR_FORM_SELECTOR,
  "script",
  "style",
  "noscript",
  "template",
...
function resetFix() {
  for (const wrapper of Array.from(fixedNodes)) {
  ...
  fixedNodes.clear();
  Array.from(fixedBlocks).forEach(restoreBlockAlignment);
  fixedBlocks.clear();
}

function getCurrentState() {
  return {
    fixed: fixedNodes.size > 0 || fixedBlocks.size > 0,
    auto: autoMode,
    mode: currentMode
  };
}
